...
const {
  logsMessagesChannel,
  updateMessage,
} = require('./app_modules/logsMessagesChannel');
...
bot.start(async (ctx) => {
  ...
  await ctx
    .reply(`Привет ${userName ? userName : 'незнакомец'} ! ${text.textStart}`, {
      parse_mode: 'html',
...
bot.command('rideon', async (ctx) => {
  try {
    ...
    if (ctx.update.message.from.username) {
      ...
      await ctx.reply(
        'Пользователи с приватным аккаунтом не могут создавать объявления'
      );
    }
  } catch (error) {
    console.error(error);
...
bot.command('delete', async (ctx) => {
  try {
    ...
    if (messageFromDb[0]) {
      await ctx.reply('Какое объявление удаляем?', {
        reply_markup: { inline_keyboard: getKeyboardForDelPost(messageFromDb) },
      });
    } else {
...
bot.on('callback_query', async (ctx) => {
  ...
  await ctx
    .deleteMessage(ctx.update.callback_query.message.message_id)
    .catch((error) => creatLogErr(error)); // удаление меню инлайн клавиатуры после нажатия любой кнопки

  async function handleQuery(callbackData, textTitle, keyboard) {
    if (cbData === callbackData) {
      await ctx.reply(textTitle, {
  ...
  if (cbData === 'meetSend') {
    ...
    try {
      if (meetStr.includes('---')) {
        ...
        const messageChannel = await ctx.telegram.sendMessage(
