...
const deletePost = async (cbData, ctx) => {
	try {
		...
		if (checkForDel) {
			...
			await Message.findOneAndDelete({ "messageChannel.message_id": forwardMess }).exec();
			...
			await ctx.reply('Вашему объявлению больше двух дней, поэтому оно будет удалено администратором канала!').catch((error) => console.log(error));
		}
