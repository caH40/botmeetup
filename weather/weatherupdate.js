	...
	try {
		...
		messageObj.forEach(async elm => {
			...
			await bot.telegram.editMessageText(
				process.env.GROUP_TELEGRAM, elm.messageGroupWeather.message_id,
				'привет!',
				(await getWeatherStart(date, location) ? await getWeatherStart(date, location) : 'Необходимо подождать, скоро я смогу предсказать погоду') + `\nUpdate: ${new Date().toLocaleString()}`,
				{ 'is_anonymous': false, 'correct_option_id': 0, 'reply_to_message_id': elm.messageGroupWeather.reply_to_message.message_id, parse_mode: 'html' }).catch(error => console.log(error));
		})
